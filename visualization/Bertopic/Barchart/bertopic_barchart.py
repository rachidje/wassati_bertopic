...
class BertopicBarchart(Barchart):
    ...
    def create_topics_per_class_df(self) -> pd.DataFrame:
        ...
        if self.sortedBy not in [None, "Frequency", "Name", "Percentage"]:
        ...
        if self.sortedBy is None and self.ascending != True:
        ...
        if self.sortedBy:
            topics_per_class = topics_per_class.sort_values(by=self.sortedBy, ascending=self.ascending)
    ...
    def visualize_topics_per_class_options(self, topics_per_class, orient="h", use_percentage=False, **kwargs):
        """
        ...
        **kwargs: Arbitrary keyword arguments for the visualize_topics_per_class_orient function.
        ...
        if orient=="h" and use_percentage==False:
            ...
            fig = self.bertopic_model.visualize_topics_per_class(topics_per_class, **kwargs)
        else:  
            # using the modified source method to do it, adding the option to print the chart vertically
            fig = BertopicBarchart.visualize_topics_per_class_orient(self.bertopic_model, topics_per_class, orient=orient, use_percentage=use_percentage, **kwargs)

        return fig

    def create_chart_per_class(self, orient="h", **kwargs):
        """
